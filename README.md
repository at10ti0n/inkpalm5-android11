...
| 3 | GSI's own Android 11 `init` (apexd, linkerconfig) | Never used: the stock 8.1 boot ramdisk `init` runs the GSI; PHH's rc does flattened apex binds. Side effect: Android 11 mount points (`/mnt/pass_through`, `/mnt/installer`, `/mnt/androidwritable`) are never created, which silently broke FUSE storage until it was diagnosed | 2-byte permissive-init patch + prepended rc, which now also creates those mount points (`a11boot/`) | Proper Android 11 boot image / vendor SELinux policy |
...
| 11 | Volume keys in reading apps | No key suits all three readers (MEASURED 2026-09-18): forward pages on Kindle/KOReader/EinkBro respectively for SPACE yes/NO/yes, DPAD_RIGHT yes/yes/NO, PAGE_DOWN NO/yes/yes | System-wide `.kl`: Vol Down = DPAD_RIGHT, Vol Up = DPAD_LEFT -- both readers page; EinkBro uses tap/swipe | Per-app remap (Key Mapper) |
